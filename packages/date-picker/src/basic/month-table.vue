<template>
  <table @click="handleMonthTableClick" @mousemove="handleMouseMove" class="el-month-table1 t-date-picker--month">
    <tbody>
      <tr v-for="(row, key) in rows" :key="key">
        <td :class="getCellStyle(cell)" v-for="(cell, key) in row" :key="key">
          <div class="t-date-picker__cell-wrapper">
            <span class="t-date-picker__cell-text">{{ t('el.datepicker.months.' + months[cell.text]) }}</span>
          </div>
        </td>
      </tr>
    </tbody>
...
export default {
  ...
  methods: {
    ...
    getCellStyle(cell) {
      ...
      style['t-date-picker__cell--now'] = today.getFullYear() === year && today.getMonth() === month;
      style.default = defaultValue.some(date => this.cellMatchesDate(cell, date));

      if (cell.inRange) {
        style['t-date-picker__cell--highlight'] = true;

        if (cell.start) {
          style['t-date-picker__cell--active'] = true;
          style['t-date-picker__cell--active-start'] = true;
        }

        if (cell.end) {
          style['t-date-picker__cell--active'] = true;
          style['t-date-picker__cell--active-end'] = true;
        }
      }
      return style;
    },
