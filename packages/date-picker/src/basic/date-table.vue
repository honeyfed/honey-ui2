      ...
      <tr class="el-date-table__row" v-for="(row, key) in rows" :class="{ current: isWeekActive(row[1]) }" :key="key">
        <td v-for="(cell, key) in row" :class="getCellClasses(cell)" :key="key">
          <div class="t-date-picker__cell-wrapper">
            <span class="t-date-picker__cell-text">
              {{ cell.text }}
            </span>
          </div>
        </td>
...
export default {
  ...
  methods: {
    ...
    getCellClasses(cell) {
      ...
      let classes = ['t-date-picker-cell'];
      if ((cell.type === 'normal' || cell.type === 'today') && !cell.disabled) {
        classes.push('available');
        if (cell.type === 'today') {
          classes.push('t-date-picker__cell--now');
        }
      } else {
        if (cell.type === 'prev-month' || cell.type === 'next-month') {
          classes.push('t-date-picker__cell--additional');
        } else {
      ...
      if (
        ...
        classes.push('t-date-picker__cell--active');
      }

      if (cell.inRange && (cell.type === 'normal' || cell.type === 'today' || this.selectionMode === 'week')) {
        classes.push('t-date-picker__cell--highlight');

        if (cell.start) {
          classes.push('t-date-picker__cell--active');
          classes.push('t-date-picker__cell--active-start');
        }

        if (cell.end) {
          classes.push('t-date-picker__cell--active');
          classes.push('t-date-picker__cell--active-end');
        }
      }

      if (cell.disabled) {
        classes.push('t-date-picker__cell--disabled');
      }
