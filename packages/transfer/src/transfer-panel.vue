<template>
  <div :class="['t-transfer__list', `t-transfer__list-${type}`]">
    <p class="t-transfer__list-header">
      <el-checkbox v-model="allChecked" @change="handleAllCheckedChange" :indeterminate="isIndeterminate">
        <div>
          <span>{{ title }} {{ checkedSummary }}</span>
        </div>
      </el-checkbox>
    </p>

    <div :class="['t-transfer__list-body', filterable ? 't-transfer__list--with-search' : '']">
      <div class="t-transfer__search-wrapper" v-if="filterable">
        <el-input
          v-model="query"
          size="small"
      ...
      <div class="t-transfer__empty" v-if="hasNoMatch || (data.length === 0 && !hasNoMatch)">
        <span v-show="hasNoMatch">{{ t('el.transfer.noMatch') }}</span>
        <span v-show="data.length === 0 && !hasNoMatch">{{ t('el.transfer.noData') }}</span>
      </div>
      <div v-else class="t-transfer__list-content">
        <el-checkbox-group
          v-model="checked"
    ...
    <p class="t-transfer__footer" v-if="hasFooter">
      <slot></slot>
    </p>
  </div>
</template>
