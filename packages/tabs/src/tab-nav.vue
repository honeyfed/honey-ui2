...
export default {
  ...
  render(h) {
    ...
    const scrollBtn = scrollable
      ? [
        <span class={['t-tabs__operations t-tabs__operations--left', scrollable.prev ? '' : 't-is-disabled']} on-click={scrollPrev}>
          <div class="t-tabs__btn--left t-tabs__btn t-size-m">
            <el-icon tdName="chevron-left"/>
          </div>
        </span>,
        <span class={['t-tabs__operations t-tabs__operations--right', scrollable.next ? '' : 't-is-disabled']} on-click={scrollNext}>
    ...
    const tabs = this._l(panes, (pane, index) => {
      ...
      return (
        <div
          class={{
            't-tabs__nav-item t-size-m': true,
            't-tabs__nav--card': type === 'card' || type === 'border-card',
            [`t-is-${this.rootTabs.tabPosition}`]: true,
            't-is-active': pane.active,
            't-is-disabled': pane.disabled,
            't-is-closable': closable,
            't-is-focus': this.isFocus
          }}
          id={`tab-${tabName}`}
          key={`tab-${tabName}`}
        ...
        >
          {type === 'card' || type === 'border-card' ? (
            <div class={{ 't-tabs__nav-item-text-wrapper': true }}>{tabLabelContent}</div>
          ) : (
            <div
              class={{ 't-tabs__nav-item-wrapper': true, 't-is-active': pane.active, 't-is-disabled': pane.disabled }}
            >
              <span class="t-tabs__nav-item-text-wrapper">{tabLabelContent}</span>
            </div>
          )}
          {(type === 'card' || type === 'border-card') && btnClose}
        </div>
      );
    });
    return (
      <div class="t-tabs__nav">
        <div
          class={[
            ...
            type === 'card' || type === 'border-card' ? 't-tabs__nav--card' : '',
            scrollable ? 'is-scrollable' : '',
            `t-is-${this.rootTabs.tabPosition}`
          ]}
        >
          {scrollBtn}
          <div class={['t-tabs__nav-scroll']} ref="navScroll">
            <div
              class={[
                't-tabs__nav-wrap t-is-smooth',
                this.rootTabs.tabPosition === 'right' || this.rootTabs.tabPosition === 'left' ? 't-is-vertical' : '',
